...
![img](https://user-gold-cdn.xitu.io/2018/6/7/163d84c797b26f89?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

上面的这个报错大家都不会陌生，报错是说没有访问权限（跨域问题）。本地开发项目请求服务器接口的时候，因为客户端的同源策略，导致了跨域的问题。

下面先演示一个没有配置允许本地跨域的的情况：

![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1042" height="251"></svg>)

![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="816" height="148"></svg>)



![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1280" height="472"></svg>)

可以看到，此时我们点击获取数据，浏览器提示我们跨域了。所以我们访问不到数据。

那么接下来我们演示设置允许跨域后的数据获取情况：

![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="752" height="256"></svg>)

**注意：配置好后一定要关闭原来的server，重新`npm run dev`启动项目。不然无效。**

![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="636" height="151"></svg>)

![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1179" height="466"></svg>)
...
![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1037" height="232"></svg>)
...
swiper需要配置哪些功能需求，自己根据文档进行增加或者删减。附上文档：[npm文档](https://www.npmjs.com/package/vue-awesome-swiper)，[swiper3.0/4.0文档](http://www.swiper.com.cn/api/index.html)，更多用法，请参考文档说明。
...
![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="519" height="168"></svg>)
...
![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="718" height="251"></svg>)
...
![img](data:image/svg+xml;utf8,<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="704" height="249"></svg>)

## 

这样的的话，打包后的图片啊js等路径就是`‘./static/img/asc.jpg’`这样的相对路径，这就不管你放在哪里，都不会有错了。但是，凡是都有但是~~~~~这里一切正常，但是背景图的路径还是不对。因为此时的相对就变成了`static/css/`文件夹下的`static/img/xx.jpg`，但是实际上`static/css/`文件夹下没有`static/img/xx.jpg`，即`static/css/static/img/xx.jpg`是不存在的。此时相对于的当前的css文件的路径。所以为了解决这个问题，要把我们css中的背景图的加个公共路径`‘../../’`，即让他往上返回两级到和`index.html`文件同级的位置，那么此时的相对路径`static/img/xx.jpg`就能找到对应的资源了。那么怎么修改背景图的这个公共路径呢，因为背景图是通过loader解析的，所以自然在loader的配置中修改，打开build文件夹下的utils文件，找到exports.cssLoaders的函数，在函数中找到对应下面这些配置：![img](https://user-gold-cdn.xitu.io/2018/9/7/165b3bd6ba562f0a?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
